<x-layouts.admin>
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <form action="{{ route('admin.deals.update', $deal->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-5 mt-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="2" placeholder="Brief description..."
                                  class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">{{ old('description', $deal->description) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        <div>
                            ...
                            <select name="discount_type" required
                                ...
                                <option value="fixed" {{ old('discount_type', $deal->discount_type) == 'fixed' ? 'selected' : '' }}>Fixed Amount (₹)</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="number" step="0.01" name="discount_value" required value="{{ old('discount_value', $deal->discount_value) }}" placeholder="e.g. 20"
                                   class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
            ...
            <x-admin.card title="Schedule & Scope">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    <div>
                        ...
                        <input type="datetime-local" name="start_date" value="{{ old('start_date', $deal->start_date?->format('Y-m-d\TH:i')) }}"
                               class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">End Date & Time</label>
                        <input type="datetime-local" name="end_date" value="{{ old('end_date', $deal->end_date?->format('Y-m-d\TH:i')) }}"
                               class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 mt-6">
                    <div>
                        ...
                        @php $dealProductIds = $deal->products->pluck('id')->toArray(); @endphp
                        <select name="product_ids[]" multiple size="6"
                                class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" {{ in_array($product->id, old('product_ids', $dealProductIds)) ? 'selected' : '' }}>{{ $product->name }}</option>
                            @endforeach
                        </select>
            ...
            <x-admin.card title="Advanced Settings">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    <div>
                        ...
                        <input type="number" name="priority" value="{{ old('priority', $deal->priority) }}" placeholder="e.g. 10"
                               class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
                    ...
                    <div class="flex flex-col justify-center space-y-4 pt-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <input type="checkbox" name="status" id="status" value="1" {{ old('status', $deal->status) ? 'checked' : '' }}

<x-layouts.admin>
    <div class="max-w-3xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800 dark:text-white">Edit Reel: {{ $reel->title ?? 'Untitled' }}</h2>
        ...
        <form action="{{ route('admin.reels.update', $reel->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <x-admin.card title="Reel Source">
                <div class="space-y-5 mt-4">
                    <div>
                        ...
                        <input type="url" name="video_url" required value="{{ old('video_url', $reel->video_url) }}" placeholder="https://www.instagram.com/reels/..."
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Custom Thumbnail</label>
                        <div x-data="{ photoName: null, photoPreview: '{{ $reel->thumbnail ? asset('storage/' . $reel->thumbnail) : '' }}' }" class="col-span-6 sm:col-span-4">
            ...
            <x-admin.card title="Reel Details">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" value="{{ old('title', $reel->title) }}" placeholder="e.g. Unboxing our new sneakers"
                               class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="product_id" 
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" {{ old('product_id', $reel->product_id) == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $reel->sort_order) }}"
                               class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none">
                    </div>

                    <div class="flex items-center space-x-3 pt-2">
                        <input type="hidden" name="status" value="0">
                        <input type="checkbox" name="status" id="status" value="1" {{ old('status', $reel->status) == '1' ? 'checked' : '' }}
